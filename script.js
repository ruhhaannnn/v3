...
const newsSources = ['AMK_Mapping', 'rnintel', 'DDGeopolitics', 'clashreport', 'presstv', 'me_observer_TG', 'spectatorindex', 'aljazeeraenglish'];
...
// Skeletons removed so baseline data shows instantly
window.loadFeeds = async function() {
    try {
        const fetchPromises = newsSources.map(u => fetchWithFastestProxy(`https://t.me/s/${u}`, 'html').then(html => ({ source: u, html })));
